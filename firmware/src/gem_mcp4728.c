...
gem_mcp_4728_write_channels(struct gem_mcp4728_channel ch_a_settings, struct gem_mcp4728_channel ch_b_settings, struct gem_mcp4728_channel ch_c_settings, struct gem_mcp4728_channel ch_d_settings) {
    uint8_t data[9] = {
        ...
        (ch_a_settings.vref << 7) | (ch_a_settings.pd << 6) | (ch_a_settings.gain << 4) | ((ch_a_settings.value >> 8) & 0xF),
        (ch_a_settings.value & 0xFF),
        (ch_b_settings.vref << 7) | (ch_b_settings.pd << 6) | (ch_b_settings.gain << 4) | ((ch_b_settings.value >> 8) & 0xF),
        (ch_b_settings.value & 0xFF),
        (ch_c_settings.vref << 7) | (ch_c_settings.pd << 6) | (ch_c_settings.gain << 4) | ((ch_c_settings.value >> 8) & 0xF),
        (ch_c_settings.value & 0xFF),
        (ch_d_settings.vref << 7) | (ch_d_settings.pd << 6) | (ch_d_settings.gain << 4) | ((ch_d_settings.value >> 8) & 0xF),
